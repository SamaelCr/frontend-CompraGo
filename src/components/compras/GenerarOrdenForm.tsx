...
import { type ApiOrder, type OrderItem } from '../../utils/api';
...
export default function GenerarOrdenForm({ onPrevStep }: Props) {
  const { accountPoints, officials, fetchAccountPoints, fetchOfficials } =
    useMasterDataStore();
  const { data: orderData, setData: setOrderData, submitOrder, reset } =
    useOrderFormStore();
  const { ivaPercentage, fetchIvaPercentage } = useSettingsStore();
  ...
  const availableAccountPoints = accountPoints.filter(
    (ap) => ap.status === 'Disponible'
  );
  const activeOfficials = officials.filter((o) => o.isActive);

  const handleItemsChange = (items: FormOrderItem[]) => {
    setOrderData({ items });
  };
  ...
  return (
    <>
      <OrderItemsManager
        ...
        ivaPercentageString={ivaPercentage.toString()}
        onIvaChange={() => {
          toast.warn('El IVA global se gestiona en la sección de Configuración del sistema.');
          const ivaInput = document.getElementById('ivaPercentage');
          if (ivaInput) ivaInput.blur();
        }}
        isIvaEditable={false} // CAMBIO: IVA no editable en la creación
      />

      <div className="mt-8 pt-6 border-t space-y-6">
        <h3 className="text-lg font-semibold text-slate-800">
          Detalles Finales de la Orden
        </h3>
        ...
        <div>
          <h3 className="block mb-2 text-sm font-medium text-slate-700">
            Impuestos Adicionales Aplicables
          </h3>
      ...
      <div className="mt-8 pt-6 border-t flex justify-between">
        ...
        <button onClick={handleFinalSubmit} disabled={isLoading} className="px-4 py-2 text-sm font-semibold rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors duration-200 bg-blue-600 text-white hover:bg-blue-700 focus:ring-blue-500 disabled:bg-slate-400 disabled:cursor-not-allowed">
