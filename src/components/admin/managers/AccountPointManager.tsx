...
import type { AccountPoint, ApiOrder } from '../../../utils/api';
import { useMasterDataStore } from '../../../stores/masterDataStore';
import GenericCRUDManager from './GenericCRUDManager';
import TableSkeleton from '../../ui/TableSkeleton';
import Alert from '../../ui/Alert';
import OrdersForAPModal from './OrdersForAPModal.tsx';
import { getOrdersByAccountPoint } from '../../../utils/api';
...
export default function AccountPointManager() {
  ...
  const errorMessage = useMasterDataStore(
    (state) => state.error.accountPoints
  );
  ...
  const [selectedAP, setSelectedAP] = useState<AccountPoint | null>(null);
  const [associatedOrders, setAssociatedOrders] = useState<ApiOrder[]>([]);
  ...
  const handleViewOrders = (item: AccountPoint) => {
    setSelectedAP(item);
    setIsModalOpen(true);
    setIsOrdersLoading(true);
    setOrdersError(null);
    getOrdersByAccountPoint(item.id)
      .then((data) => {
  ...
  if (errorMessage) {
    return (
      <div className="mt-6">
        <Alert
          title="Ocurrió un error al cargar los Puntos de Cuenta"
          variant="danger"
        >
          <p>{errorMessage}</p>
          <div className="mt-4">
  ...
  return (
    <>
      <GenericCRUDManager<AccountPoint>
        ...
        items={accountPoints}
        api={{
          create: createAccountPoint,
          update: updateAccountPoint,
          delete: deleteAccountPoint,
        }}
        ...
        renderTableRow={(item, handleEdit, handleDelete) => (
          <tr
            ...
            title="Haga clic para ver órdenes asociadas"
          >
          ...
          <>
            ...
            <div>
              ...
              <input
                ...
                value={formData.programmaticCategory}
                onChange={handleChange}
                className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg"
              />
            </div>
